...
def db(app):
    ...
    with app.app_context():
        ...
        with rollback_isolation(_db):
            # Seed the baseline EUR currency inside the test's own transaction so
            # the route/repository paths see it on the same connection; it rolls
            # back with the rest of the test's writes.
